            ...
            using (OleDbConnection connection = new OleDbConnection(Settings.ConnectionString))
            {
                ...
                if (_tableCreated)
                {
                    ...
                    using (OleDbCommand command = connection.CreateCommand())
                    {
                        ...
                        query.Append($"   ,to_date({DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss").ToSqlString()}, 'yyyy-MM-dd HH24:MI:SS') ");
                    ...
                    throw new Exception($"It was not possible to create the table {Settings.DatabaseTableToStoreLog}");
            }
        }
    }
}
